...
class PlayStoreInterstitialAdManager @Inject constructor(
    ...
    override fun configure(activity: ComponentActivity) {
        activity.lifecycleScope.launch {
            combine(
                ...
                Triple(
                    adsEnabled && readerEntryInterstitialAdsEnabled,
                    showInterval,
                    adSessionConfig
                )
            }
                .collectLatest { (enabled, showInterval, _) ->
                    configureAds(
                        activity = activity,
                        enabled = enabled,
    ...
    private fun configureAds(
        ...
        readerEntriesSinceLastShown = readerEntriesSinceLastShown.coerceAtMost(
            this.showInterval
        )
    ...
    override fun onReaderEntered(activity: ComponentActivity) {
        if (!adsEnabled || !privacyConsentManager.canRequestAds(activity)) return
