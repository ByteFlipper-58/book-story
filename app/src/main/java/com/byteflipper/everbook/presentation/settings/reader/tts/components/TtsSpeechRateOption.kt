...
// Ticks every 0.1x across the range the engine accepts.
private val STEP_COUNT =
    ((TtsPreferences.MAX_SPEECH_RATE - TtsPreferences.MIN_SPEECH_RATE) * 10).toInt() - 1

@Composable
fun TtsSpeechRateOption() {
    val mainModel = hiltViewModel<MainModel>()
    val state = mainModel.state.collectAsStateWithLifecycle()
    val interactionSource = remember { MutableInteractionSource() }

    val speechRate = state.value.ttsSpeechRate
    val displayValue = String.format(Locale.US, "%.1f", speechRate)
    ...
    Row(
        ...
        Column(Modifier.fillMaxWidth(0.2f)) {
            SettingsSubcategoryTitle(
                title = stringResource(id = R.string.tts_speech_rate_option),
                padding = 0.dp
            )
        ...
        Slider(
            ...
            onValueChange = {
                ...
                mainModel.onEvent(MainEvent.OnChangeTtsSpeechRate(rounded))
            },
